...
def agreger_annee_par_enseigne(annee, correspondance, journal=print):
    ...
    Les stations d'autoroute sont écartées : elles se vendent nettement plus
    cher, et les réseaux n'en comportent pas la même proportion. Les inclure
    imputerait à la politique commerciale d'une enseigne ce qui ne tient qu'à
    l'emplacement de ses stations.
    """
    import collections
    ...
    somme = collections.defaultdict(lambda: np.zeros(nb_jours))
    compte = collections.defaultdict(lambda: np.zeros(nb_jours, dtype=np.int32))
    retenues = ecartees = 0
    ...
    with zipfile.ZipFile(chemin_archive(annee)) as zip_ouvert:
        with zip_ouvert.open(zip_ouvert.namelist()[0]) as flux:
            ...
            for evenement, element in contexte:
                ...
                enseigne = correspondance.get(str(element.get("id") or "").strip())
                autoroute = (element.get("pop") or "R").upper() == "A"
                if enseigne is None or autoroute:
                    ecartees += 1
                    element.clear(); racine.clear()
                    continue
                retenues += 1
                ...
                for nom, par_jour in releves.items():
                    ...
                    if not connus.any():
                        continue
                    cle = (enseigne, nom)
                    somme[cle][connus] += serie[connus]
                    compte[cle][connus] += 1

                element.clear(); racine.clear()

    journal(f"    {annee} : {retenues} stations retenues, {ecartees} écartées")

    lignes = []
    for (enseigne, nom), valeurs in somme.items():
        effectifs = compte[(enseigne, nom)]
        couverture = int(effectifs[:nb_jours_utiles].max(initial=0))
        if couverture < 30:
            continue
        seuil = max(20, int(couverture * config.PART_MINIMALE_COUVERTURE))
        for indice in range(nb_jours_utiles):
            n = int(effectifs[indice])
            if n < seuil:
                continue
            lignes.append((
                (debut_annee + dt.timedelta(days=indice)).isoformat(),
                enseigne, nom, round(valeurs[indice] / n, 4), n,
            ))
    return lignes
